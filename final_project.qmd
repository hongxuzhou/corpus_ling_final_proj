...
This study exploys a specialised corpus conprising ten books by Neil Gaiman, slected to represent the author's rage across audience demographics. The ten books are:
...
The the cop was designed with the multi-tired structure to accelarate analysis at different levels of linguistic granularity:
...
Word length often correlates with text complexity — longer words typically appear in more advanced or specialised text [@biber1988]. Figure @fig-word_length_vis presents the percentage of words at each length across Gaiman's works. This visualisation reveals a remarkably consistent pattern across all books, with most words averaging around 4 characters regardless of the intended audience. This suggests that at the the basic word length level, Gaiman maintains a relatively stable style.
...
This result is worthy of detailed report.
...
The complete list of profanity can be found in Appendix. @fig-swear_analysis shows the frequency of profanity per 1,000 words for each book. The analysis uncover clear differences between adults and children's books. Adult-oriented titles contain substantially more profanity expressions across
...
I am not sure about how to report this part. Sentiment and profanity analysis provide most intuitive results for differentiating age groups. But They rely on strong examples to be persuasive, which can be redundant and lengthy. So, the visualisations will be kept, but the structure and content of this part need to be carefully considered.
...
I am not sure if I should keep all three visualisations in the POV analysis section. I think the first and third ones are bit overlapped. Also, I don't know what should be the key observation to report in this part.
...
The modal verb analysis has the similar problem with POV analysis. I think the first visulasation, the distribution of modal verb categories, is more about descriptive, while the narrative voice authority might be of more values.
...
The result of PCA, especially the spectrum, should be a highlight of analysis and even the whole report. It provides some counter-intuitive results but also nuanced features. So, this part should be more careful and critical when reporting. A good report should use PCA to connect and integrate the analysis all above and beyond. This would be a strong foundation for the following discussion and conclusion sections.
