...
Result reporting: the basic nature of the corpus
...
As the chart shows, Gaiman's writing shows a consistent pattern across his works. Most words are at around 4 characters, which reflect the common pattern of English. The shorted appears in Ocean, while the longest in NW.
...
Admit the limitation: TTR is sensitive to text length, and so simply using TTR cannot differentiate if a book is for children or adults, so we need more extensive and in-depth analysis.
...
Briefly explain this keyword results.
...
The result of sentence length distribution may of fewer analytical values, but builds the base for coming analysis. So, I put it at the beggning of this section.
...
first report the general pattern then focus on the specific ones. It may be interesting to connect it with UD results above.
